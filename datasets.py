...
from util.toolkit import count_parameters

__all__ = ['build_continual_dataloader', 'get_dataset', 'build_upstream_continual_dataloader', 'build_transform', 'build_cifar_transform']

# def oversample_batch(batch, target_batch_size):
#     if len(batch) >= target_batch_size:
#         return batch

#     pad_size = target_batch_size - len(batch)
#     extra = random.choices(batch, k=pad_size)
#     return batch + extra

# from torch.utils.data._utils.collate import default_collate

# def oversample_collate_fn(target_batch_size):
#     def fn(batch):
#         batch = oversample_batch(batch, target_batch_size)
...
def get_dataset(dataset, transform_train, transform_val, args, target_transform=None):
    if dataset == 'CIFAR100':
        ...
        dataset_train = CUB200(args.data_path, train=True, download=False, transform=transform_train, target_transform=target_transform).data
        dataset_val = CUB200(args.data_path, train=False, download=False, transform=transform_val, target_transform=target_transform).data
...
def build_core50(args):
    ...
    for task in range(num_tasks):
        ...
        cls_so_far = 25
        args.nb_classes = 50
        # cls_so_far = known_class+data_manager.get_task_size(task)
        train_dataset = data_manager.get_dataset(np.arange(known_class, cls_so_far), source="train", mode="train")
        train_loader = DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers)
        test_dataset = data_manager.get_dataset(np.arange(0, cls_so_far), source="test", mode="test")
        test_loader = DataLoader(test_dataset, batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers)
        open_test_dataset = data_manager.get_dataset(np.arange(cls_so_far,50), source="test", mode="test")
        open_test_loader = DataLoader(open_test_dataset, batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers)
        dataloader.append({'train':train_loader, 'val':test_loader, 'open':open_test_loader})
